...
1. The Agent Gateway dataplane selects the target and forwards the caller's original bearer token.
...
| State Concern | Direct Routing | Bridge Routing |
| ------------- | -------------- | -------------- |
| MCP session | A client can initialize a Streamable HTTP session and reuse its `Mcp-Session-Id`. | The bridge does not extend the caller's session into a remote shard. The hub and leaves remove `Mcp-Session-Id` before the leaf calls its local gateway. |
| Upstream association | Selector routing associates the session with one discovered endpoint. The dataplane preserves the association across gateway replicas. | Each remote request is an independent, stateless MCP request. |
| Protocol handling | The selected target handles methods within the negotiated client session. | The hub handles `initialize` and `ping` locally. It accepts supported JSON-RPC notifications without forwarding session state. |
| Stateful methods | The selected target can use server-side MCP session state. | Methods that require server-side MCP session state, client state, or resource-path routing are unavailable. |
