...
| Agentgateway controller | Programs Agentgateway resources from Kubernetes Gateway API and Agentgateway custom resources. |
| Agentgateway dataplane | Accepts `/mcp` traffic and applies authentication, authorization, rate limiting, and target routing. |
...
flowchart LR
    ...
    edge -->|"HTTP /mcp"| gateway["Agentgateway Dataplane"]
...
1. Agentgateway selects the target and forwards the caller's original bearer token.
...
Agentgateway combines the catalogs from the targets that a tenant can access. It uses target-qualified names to route a tool, prompt, or resource request to the selected server. An unavailable target does not prevent healthy targets from remaining available in a catalog response.

Selector targets match Kubernetes Services by namespace and labels. Each selected Service exposes an MCP port with `appProtocol: agentgateway.dev/mcp`. Agentgateway discovers the Service endpoints and maintains session affinity to the selected endpoint when a client uses an MCP session.
...
The bridge supports JSON responses and streamed server-sent events (SSE) for routed invocations. The [validated capabilities](validated-capabilities.mdx) page identifies methods that the stateless bridge does not support.

## Sessions and State

Direct and bridged requests have different state boundaries.

For direct routing, a client can initialize a streamable HTTP MCP session and reuse its `Mcp-Session-Id` value. Selector-mode routing keeps that session associated with one discovered upstream endpoint. The tested dataplane preserves this upstream association when a later request reaches another gateway replica.
...
A bridge leaf temporarily owns the response state for an active SSE stream. The hub reads ordered frames from that leaf until completion, cancellation, or idle expiry. This temporary stream ownership does not create an MCP session in the remote shard.
...
For HTTPS static targets, Agentgateway initiates TLS to the upstream. For selector targets and bridge leaf-to-gateway traffic, the documented in-cluster path uses HTTP.
