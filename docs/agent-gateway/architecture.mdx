...
DSX Agent Gateway provides a policy enforcement and routing layer between MCP clients and MCP servers. Without bridge routing, the gateway sends requests to targets that it can reach. The optional bridge extends selected requests to remote gateway shards through the DSX Event Bus.
