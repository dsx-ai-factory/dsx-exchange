...
For application code, use an MCP client library that supports Streamable HTTP. Choose the MCP Inspector workflow for interactive testing or the manual protocol workflow for connectivity tests and troubleshooting.
...
The following examples use the MCP Inspector to interactively try out the gateway. You can either use the MCP Inspector (fastest), or the manual protocol workflow (slower, but more verbose and detailed).
