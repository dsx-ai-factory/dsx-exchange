...
| Symptom | Start Here |
| ------- | ---------- |
| Gateway not programmed. | [Troubleshoot Gateway Programming](#troubleshoot-gateway-programming) |
| Gateway rejects authentication or target access. | [Troubleshoot Authentication and Target Access](#troubleshoot-authentication-and-target-access) |
| An MCP target is missing or unavailable. | [Troubleshoot Missing MCP Targets](#troubleshoot-missing-mcp-targets) |
| A request returns HTTP 429, or the rate-limit path fails. | [Troubleshoot Rate Limiting and Valkey](#troubleshoot-rate-limiting-and-valkey) |
| A bridge Pod is not ready, or a remote shard is unavailable. | [Troubleshoot the Bridge](#troubleshoot-the-bridge) |
