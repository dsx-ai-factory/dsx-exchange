...
| --------- | ------------- | ----- |
| DSX Agent Gateway dataplane | 3 replicas | `gateway.replicaCount` |
...
## Observe DSX Agent Gateway

DSX Agent Gateway can expose Prometheus metrics for the dataplane, Agent Gateway controller, rate-limit service, bridge, and bundled Valkey. It can also export OpenTelemetry traces for the dataplane, rate-limit service, and bridge. Refer to the [configuration reference](configuration-reference.mdx) to connect these integrations to your observability platform.
...
- No DSX Agent Gateway dataplane metrics for 10 minutes.
...
Every chart-deployed container writes logs to standard output or standard error. The DSX Agent Gateway dataplane and Agent Gateway controller use their native structured logs. The rate-limit service and bridge use JSON logs. Use component labels to select logs:
